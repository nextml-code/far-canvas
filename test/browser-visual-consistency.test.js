...
// Skip this test in CI environment due to ES module import issues
const CI = process.env.CI === "true";
if (CI) {
  describe.skip("Browser Visual Consistency Test (Puppeteer with HTTP Server)", () => {
    test("skipped in CI environment", () => {
      console.log(
        "Browser visual consistency test skipped in CI due to ES module import issues"
      );
    });
  });
} else {
  ...
  describe("Browser Visual Consistency Test (Puppeteer with HTTP Server)", () => {
    ...
    beforeAll(async () => {
      ...
      if (!fs.existsSync(htmlPath)) {
        throw new Error(
          `FATAL: browser-test.html does not exist at ${htmlPath}`
        );
      }
