...
describe("Bug detection tests", () => {
  test("verifies drawImage with 4 args transforms correctly", () => {
    ...
    const farCtx = far(farCanvas, { x: offsetX, y: offsetY, scale: scale }).getContext("2d");
    farCtx.fillStyle = "white";
    farCtx.fillRect(0, 0, width/scale, height/scale);
    farCtx.drawImage(sourceCanvas, 10, 15, 40, 30);
    
    // Compare
    const imageData1 = vanillaCanvas.getContext("2d").getImageData(0, 0, width, height);
    const imageData2 = farCanvas.getContext("2d").getImageData(0, 0, width, height);
  ...
  test("verifies gradient transformations are correct", () => {
    ...
    const vanillaData = vanillaCanvas.getContext("2d").getImageData(100, 100, 1, 1).data;
    const farData = farCanvas.getContext("2d").getImageData(100, 100, 1, 1).data;
  ...
  test("verifies line dash offset transforms correctly", () => {
    ...
    const vanillaData = vanillaCanvas.getContext("2d").getImageData(100, 40, 50, 20);
  ...
  test("verifies shadow properties transform correctly", () => {
    ...
    const vanillaData = vanillaCanvas.getContext("2d").getImageData(130, 130, 20, 20);
    ...
    for (let i = 0; i < farData.data.length; i += 4) {
      if (farData.data[i] < 255 || farData.data[i+1] < 255 || farData.data[i+2] < 255) {
