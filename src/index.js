    ...
    */
	...
	return {
        ...
        arc(x, y, radius, startAngle, endAngle, counterclockwise) {
            return _context.arc(s.x(x), s.y(y), s.distance(radius), startAngle, endAngle);
        },
        arcTo(x1, y1, x2, y2, radius) {
            return _context.arcTo(s.x(x1), s.y(y1), s.x(x2), s.y(y2), s.distance(radius));
        },
        beginPath() {
            return _context.beginPath();
        },
        bezierCurveTo(cp1x, cp1y, cp2x, cp2y, x, y) {
            return _context.bezierCurveTo(s.x(cp1x), s.y(cp1y), s.x(cp2x), s.y(cp2y), s.x(x), s.y(y));
        },
        clearRect(x, y, width, height) {
            return _context.clearRect(s.x(x), s.y(y), s.distance(width), s.distance(height));
        },
        clip(...args) {
            // NOTE see clip
            if (args.length === 0) {
        ...
        createImageData(...args) {
            if (args.length === 1) {
                ...
                return _context.createImageData(s.distance(width), s.distance(height), settings);
            }
        },
        ...
        createRadialGradient(x0, y0, r0, x1, y1, r1) {
            return _context.createRadialGradient(s.x(x0), s.y(y0), s.distance(r0), s.x(x1), s.y(y1), s.distance(r1));
        },
        ...
        ellipse(x, y, radiusX, radiusY, rotation, startAngle, endAngle, counterclockwise) {
            return _context.ellipse(s.x(x), s.y(y), s.distance(radiusX), s.distance(radiusY), rotation, startAngle, endAngle, counterclockwise);
        },
        fill(...args) {
            // NOTE see clip
            if (args.length === 0) {
        ...
        rect(x, y, width, height) {
            return _context.rect(s.x(x), s.y(y), s.distance(width), s.distance(height));
        },
        resetTransform() {
        ...
        roundRect(x, y, width, height, radii) {
            return _context.roundRect(s.x(x), s.y(y), s.distance(width), s.distance(height), s.distance(radii));
        },
        save() {
        ...
        strokeRect(x, y, width, height) {
            return _context.strokeRect(s.x(x), s.y(y), s.distance(width), s.distance(height));
        },
